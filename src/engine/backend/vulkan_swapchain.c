...
static inline bool SelectVkPresentModeKHR(const VulkanDevice device[static 1], VkPresentModeKHR present_mode[static 1])
{
...
static inline bool SelectSwapchainExtent(const VulkanDevice device[static 1], const uint32_t width, const uint32_t height, VkExtent2D extent[static 1])
{
    ...
    {
        const uint32_t min_width = capabilities.minImageExtent.width > width ? capabilities.minImageExtent.width : width;
        extent->width            = capabilities.maxImageExtent.width < min_width ? capabilities.maxImageExtent.width : min_width;

        const uint32_t min_height = capabilities.minImageExtent.height > height ? capabilities.minImageExtent.height : height;
        extent->height            = capabilities.maxImageExtent.height < min_height ? capabilities.maxImageExtent.height : min_height;
    }
    else
    {

...
bool CreateVulkanBuffer(const VulkanDevice device[static 1], const VulkanBufferCreateInfo create_info[static 1], VulkanBuffer buffer[static 1])
{
...

    {
        ...
        const VkMemoryAllocateInfo buffer_alloc_info = {
            ...
            .memoryTypeIndex = FindMemoryType(device->physical_device, mem_requirements.memoryTypeBits, create_info->properties)};
...
{
    const VulkanBufferCreateInfo create_info = {.size       = size,

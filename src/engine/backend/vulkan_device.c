...
void DestroyVulkanDevice(VulkanDevice device [static 1]) {
...
bool CreateVkDebugUtilsMessengerEXT(
    ...
    VkDebugUtilsMessengerEXT                    debug_messenger     [static 1]
) {
...
static inline bool CreateVkSurfaceKHR(
    const VkInstance                            instance, 
    const VkAllocationCallbacks*                allocator, 
    const GLFWWindow        window              [static 1],
...
static inline uint32_t RankPhysicalDevice(const VkSurfaceKHR surface, const VkPhysicalDevice physical_device) {
    ...
    switch (physical_device_properties.properties.deviceType) {
        ...
        case VK_PHYSICAL_DEVICE_TYPE_MAX_ENUM:          return 0;
        default:                                        return 0;
    }
}

static inline bool SelectVkPhysicalDevice(
    const VkInstance                        instance, 
    const VkSurfaceKHR                      surface, 
    VkPhysicalDevice                        physical_device         [static 1]
) {
...
static inline bool CreateVkDevice(
    const VkPhysicalDevice                  physical_device, 
    uint32_t                                queue_index_count,
    uint32_t*                               queue_indices,
    VkDevice                                device                  [static 1]
) {
    uint32_t byte_count = 0;
...
bool CreateVulkanDevice(
    ...
    const FindQueueFamilyIndexInfo find_info = {
        .queue_capability_count = create_info->queue_description_count,
